...
void Waveguide::setFrequency(float frequency)	{
	...
	_periodInMilliseconds = 1000.0f/frequency;
...
void Waveguide::updateFilterCoeffs(float frequency)	{
	...
	a1_lp = 0;
	a2_lp = 0;
	b0_lp = 1.0f - frequency;
	b1_lp = frequency;
	b2_lp = 0;
	
	/*
	float w0 = 2*M_PI*8000/44100;       //Angular Freq. (Radians/sample) 
	float alpha = sin(w0)/(2*0.707);      //Filter Width
	float A  = sqrt(10^(-6/20));  //Amplitude
